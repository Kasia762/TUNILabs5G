...
plt.savefig('signal_strength_analysis.png', dpi=300)
#plt.show()

with open('average_signal_strength.txt', 'w') as f:
    f.write("Overall Signal Strength:\n")
    f.write(f"Overall - Average 4G RSRP: {avg_RSRP4G_all:.2f} dBm\n")
    f.write(f"Overall - Average 5G RSRP: {avg_RSRP5G_all:.2f} dBm\n")
    f.write(f"Overall - 4G Coverage: {coverage_4G_all:.2f}%\n")
    f.write(f"Overall - 5G Coverage: {coverage_5G_all:.2f}%\n\n")
    
    f.write("Average Signal Strength per Floor:\n")
    for index in range(3):
        f.write(f"Floor {index} - Average 4G RSRP: {average_RSRP4G[index]:.2f} dBm\n")
        f.write(f"Floor {index} - Average 5G RSRP: {average_RSRP5G[index]:.2f} dBm\n")
        f.write(f"Floor {index} - 4G Coverage: {coverage_4G[index]:.2f}%\n")
        f.write(f"Floor {index} - 5G Coverage: {coverage_5G[index]:.2f}%\n\n")
